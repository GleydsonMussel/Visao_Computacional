...
def clean_data(destiny_folder, file_name, path_to_picke_dict, keys_to_clean):
    pickle_dict_import = load_pickle_data(path_to_picke_dict)
    pickle_dict_export = {}
    for key in keys_to_clean:
        data_to_clean = pd.DataFrame({key:pickle_dict_import[key]})
        data_to_clean = remove_outliers(data_to_clean, key)
    ...
    save_as_pickle_data(pickle_dict_export, destiny_folder, file_name+".pkl")
