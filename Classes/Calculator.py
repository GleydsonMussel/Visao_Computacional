...
class Calculator: 
    ...
    @staticmethod
    # Calcula o ponto d @staticmethodo centro da região de interesse
    ...
    def calc_speed(x, y, w, h, cxant, cyant, iteracao, fps, distancia_acumulada_X, altura_acumulada, fatConvPxM, pasta_output):
        ...
        with open("./Textual_Inputs_Tracker_Processing/paths_to_tracker_processing.txt", 'r') as arquivo:
            caminhos = arquivo.readlines()
            caminhos = [caminho.split(" ")[0].replace("\n", "") for caminho in caminhos]
            arquivo.close()
        
        [Manipulate_Files.save_data(dado, pasta_output+caminho) for dado, caminho in zip(dados, caminhos)]
    ...
    # Calcula a posição do centro   @staticmethoddo marcador ArUco
    @staticmethod
    ...
    # Calcula os vetores de r   @staticmethodotação e translação relacionados ao ArUco com relação a câmera
    @staticmethod
    ...
    def calc_speed_ArUco(destiny_folder, file_name, marker_positions, times_to_each_marker, ids_wanted_markers):
        ...
        Manipulate_Files.save_as_pickle_data(speeds, destiny_folder, file_name)
